...
--2 uzduotis pasikartojimas
insert into rysio_planai
values(1, 'tele2', 5, 10)

insert into klientas
values(1, 'Jonas', 'Jonaitis', 860607544, 1)

create trigger naujas_klientas2
on klientas
for insert
as
begin
insert into saskaita
select kliento_nr, month(getdate()), year(getdate()), 0 from inserted
end
...
insert into klientas
values(2, 'Kevinas', 'Durantas', 867554063, 2)

select * from rysio_planai
select * from klientas
select * from saskaita

--3 uzduotis pasikartojimas (Kai insertina i isklotine naujus duomenis, turi buti atnaujinta saskaitu lentele su naujais duomenimis)
create trigger atnaujinimas
on isklotine
for update
as
begin
update saskaita
set suma = suma + (select sujungimo_kaina + (PSK * (select pokalbio_trukme from inserted)) from isklotine
inner join klientas on isklotine.kliento_id = klientas.id
inner join rysio_planai on rysio_planai.id = klientas.rysio_planas
where isklotine.kliento_id = klientas.id and klientas.rysio_planas =rysio_planai.id)
end

insert into isklotine
values(1, '2003-04-16', '14:20:20', 863491234, 30)

update isklotine
set pokalbio_trukme = pokalbio_trukme +20

select * from isklotine

--kartais tau padropint duomenu baze reikes jei kokius foreign key erorus metis ar seip kas neveiks xd
